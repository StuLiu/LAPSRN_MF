...
class LapSRN(object):
	...
	def test(self, config):
		...
		while (i+1) * self.batch_size < time_dimen:
			Output = self.sess.run([self.O], feed_dict={self.X: input[i * self.batch_size:(i + 1) * self.batch_size],
		                                                self.keep_prob: 1})
			print(np.array(Output).shape)
			i += 1
		Output = self.sess.run([self.O], feed_dict={self.X: input[i * self.batch_size:],
		                                            self.keep_prob: 1})
		print(np.array(Output).shape)
		# idx = len(input)//self.batch_size
		# writedata = []
